# Copyright 2012-2021 Michael Pozhidaev <msp@luwrain.org>
# The LUWRAIN Project, GPL v.3

VERSION=1.9.01
REGISTRY_VERSION=1.9.01
GITS='base browser extensions i18n io interaction-javafx linux luwrain packs pim player publisher reader registry studio windows'
PROPERTIES_FILE=release.properties
JUNIT_COMPONENTS='base luwrain linux browser io pim app-notepad'
SDK_JARS='base browser io pim studio'
SRC_DEST_DIR=/tmp/luwrain-src

# Used for compilation, everything must be listed regardless the target platform
LANGS='en fr ro ru'
APPS='commander contacts mail news notepad telegram twitter viewer vk'
EXTENSIONS='cmdtts emacspeak mssapi plmp3 rhvoice speechd voiceman'
COMPONENTS='base io browser linux packs pim player publisher reader studio windows interaction-javafx'

WIN_ARCH=64
WIN_LANGS='en ro ru'
WIN_COMPONENTS='base browser interaction-javafx io packs pim player publisher reader studio windows'
WIN_APPS='commander contacts mail news notepad telegram twitter viewer vk'
WIN_EXTENSIONS='mssapi plmp3 rhvoice'
WIN_PROPERTIES='luwrain player reader windows'

LINUX_ARCH='64'
LINUX_LANGS='en ro ru'
LINUX_COMPONENTS='base browser interaction-javafx io linux packs pim player publisher reader studio'
LINUX_APPS='commander contacts mail news notepad telegram twitter viewer vk'
LINUX_EXTENSIONS='cmdtts emacspeak plmp3 rhvoice speechd voiceman'
LINUX_PROPERTIES='linux luwrain player reader'
LINUX_SCRIPTS='linux'

ISO_ARCH='64'
ISO_LANGS='en ro ru'
ISO_COMPONENTS='base browser interaction-javafx io linux packs pim player publisher reader studio'
ISO_APPS='commander contacts mail news notepad telegram twitter viewer vk'
ISO_EXTENSIONS='cmdtts emacspeak plmp3 rhvoice speechd voiceman'
ISO_PROPERTIES='linux luwrain player reader'
ISO_SCRIPTS='linux'
